...
from baselines.DaSiamRPN.code.utils import get_subwindow_tracking
...
def SiamRPN_init(im, target_pos, target_sz, net, compute_device: torch.device):
    state = dict()
    p = TrackerConfig()
    p.update(net.cfg)
...
def SiamRPN_track(state, im, compute_device: torch.device):
    p = state['p']
    net = state['net']
    ...
    x_crop = x_crop.to(compute_device)
